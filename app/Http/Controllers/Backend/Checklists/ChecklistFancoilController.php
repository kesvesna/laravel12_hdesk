...
use App\Http\Filters\Checklists\Fancoil\ChecklistFancoilFilter;
use App\Http\Requests\Checklists\Fancoil\ChecklistFancoilFilterRequest;
use App\Http\Requests\Checklists\Fancoil\StoreChecklistFancoilFormRequest;
use App\Http\Requests\Checklists\Fancoil\StoreChecklistFancoilFromTrkEquipmentFormRequest;
use App\Http\Requests\Checklists\Fancoil\StoreChecklistFancoilFromTrkRoomFormRequest;
use App\Http\Requests\Checklists\Fancoil\UpdateChecklistFancoilFormRequest;
...
class ChecklistFancoilController extends Controller
{
    public function __construct()
    {
        $this->middleware(['auth', 'verified', 'profiled', 'not_blocked']);
        $this->authorizeResource(ChecklistFancoil::class, 'checklists_fancoil');
    }

    /**
     * Display a listing of the resource.
     */
    public function index(ChecklistFancoilFilterRequest $request): Response
    {

        $data = $request->validated();

        $filter = app()->make(ChecklistFancoilFilter::class, ['queryParams' => array_filter($data)]);

        $checklists = ChecklistFancoil::filter($filter)
            ->orderBy('created_at', 'desc')
            ->paginate(config('backend.checklists_fancoil.pagination'));

        return \response()->view('backend.checklists.fancoil.index', [
    ...
    public function create(): Response
    {
        ...
        return \response()->view('backend.checklists.fancoil.create', [
     ...
     */
    public function store(StoreChecklistFancoilFormRequest $request): RedirectResponse
    {
        Log::info('user try to store new checklist fancoil', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
        ]);
        ...
        if ($request->isMethod('post')) {
            try {
                ...
                $checklist = ChecklistFancoil::create([
                ...
                return redirect()->route('checklists_fancoil.index')->with('success', 'Данные сохранены.');
    ...
    {
        $this->authorize('create', ChecklistFancoil::class);

        $work_types = WorkName::whereIn('name', WorkName::CHECKLIST_WORK_TYPES)->get();

        return \response()->view('backend.checklists.fancoil.create_from_trk_equipment', [
            'executors' => User::orderBy('name')->get(),
            'work_types' => $work_types,
     ...
     * Store a newly created resource in storage.
     */
    public function store_from_trk_equipment(StoreChecklistFancoilFromTrkEquipmentFormRequest $request, TrkEquipment $trk_equipment): RedirectResponse
    {
        $this->authorize('create', ChecklistFancoil::class);

        Log::info('user try to store new checklist fancoil from trk equipment', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
            'trk_equipment' => $trk_equipment,
        ]);
    ...
    public function create_from_trk_room(TrkRoom $trk_room): Response
    {
        ...
        return \response()->view('backend.checklists.fancoil.create_from_trk_room', [
            'executors' => User::orderBy('name')->get(),
            'work_types' => $work_types,
     ...
     * Store a newly created resource in storage.
     */
    public function store_from_trk_room(StoreChecklistFancoilFromTrkRoomFormRequest $request, TrkRoom $trk_room): RedirectResponse
    {
        $this->authorize('create', ChecklistFancoil::class);

        Log::info('user try to store new checklist fancoil from trk room', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
            'trk_room' => $trk_room,
        ]);
        ...
        if ($request->isMethod('post')) {
            try {
                ...
                $system_fancoil = System::where('name', System::AIR_CONDITION)->first();
     ...
     */
    public function show(ChecklistFancoil $checklists_fancoil): Response
    {
        return \response()->view('backend.checklists.fancoil.show', [
            'checklist' => $checklists_fancoil,
        ]);
    }
     ...
     */
    public function edit(ChecklistFancoil $checklists_fancoil): Response
    {
        $work_types = WorkName::whereIn('name', ['ТО 4', 'ТО 5', 'ТО 6'])->get();
        $system_fancoil = System::where('name', System::AIR_CONDITION)->first();


        $trk_building_ids = TrkRoom::where('trk_id', $checklists_fancoil->trk_room->trk->id)
            ->groupBy('building_id')
            ->pluck('building_id')
            ->toArray();
        ...
        $trk_room_ids = TrkRoom::where('trk_id', $checklists_fancoil->trk_room->trk->id)
            ->pluck('room_id')
            ->toArray();

        $rooms = Room::whereIn('id', $trk_room_ids)->orderBy('name')->get();

        $trk_floor_ids = TrkRoom::where('trk_id', $checklists_fancoil->trk_room->trk->id)
            ->groupBy('floor_id')
            ->pluck('floor_id')
            ->toArray();
     ...
     */
    public function update(UpdateChecklistFancoilFormRequest $request, ChecklistFancoil $checklists_fancoil): RedirectResponse
    {

        Log::info('user try to update checklist fancoil', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
            'checklist fancoil' => $checklists_fancoil
        ]);
        ...
        if ($request->isMethod('patch')) {
            try {
                ...
                $trk_room = TrkRoom::where('trk_id', $checklists_fancoil->trk_room->trk->id)
                    ->where('building_id', $data['building_id'])
                    ->where('floor_id', $data['floor_id'])
                    ->where('room_id', $data['room_id'])
                    ->first();
                ...
                if (empty($trk_room->id)) {
                    ...
                    $trk = Trk::find($checklists_fancoil->trk_room->trk->id);
                ...
                if (empty($equipment_name->id)) {
                    ...
                    return redirect()->back()->with('error', 'Нет фанкойла с таким названием: ' . $data['equipment_id'])->withInput();
                }
                ...
                if (empty($trk_equipment->id)) {
                    ...
                    return redirect()->back()->with('error', 'Нет такого фанкойла: ' . $trk_room->trk->name . ', ' . $trk_room->building->name . ', ' . $trk_room->floor->name . ' - ' . $equipment_name->name)->withInput();
                }

                DB::beginTransaction();

                $avr = $checklists_fancoil->avr->first();

                foreach($avr->avr_equipments as $avr_equipment)
                {
                    if(
                        $avr_equipment->trk_equipment_id == $checklists_fancoil->trk_equipment_id
                ...
                AvrWork::where('avr_id', $avr->id)
                    ->where('trk_equipment_id', $checklists_fancoil->trk_equipment_id)
                    ->forceDelete();
     ...
     */
    public function destroy(ChecklistFancoil $checklists_fancoil): RedirectResponse
    {
        Log::info('user try to delete ChecklistFancoil', [
            'user' => Auth::user()->name,
        ...
        try {
            ...
            $avr_id = $checklists_fancoil->avr->first()->id;

            DocCommunication::where('from_id', $checklists_fancoil->id)
                ->where('from_type', ChecklistFancoil::class)
                ->where('to_id', $avr_id)
            ...
            $checklists_fancoil->delete();

            DB::commit();

            return redirect()->route('checklists_fancoil.index')->with('success', 'Данные удалены.');
