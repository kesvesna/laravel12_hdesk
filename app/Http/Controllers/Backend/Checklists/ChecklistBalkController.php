...
use App\Http\Filters\Checklists\Balk\ChecklistBalkFilter;
use App\Http\Requests\Checklists\Balk\ChecklistBalkFilterRequest;
use App\Http\Requests\Checklists\Balk\StoreChecklistBalkFormRequest;
use App\Http\Requests\Checklists\Balk\StoreChecklistBalkFromTrkEquipmentFormRequest;
use App\Http\Requests\Checklists\Balk\StoreChecklistBalkFromTrkRoomFormRequest;
use App\Http\Requests\Checklists\Balk\UpdateChecklistBalkFormRequest;
...
class ChecklistBalkController extends Controller
{
    public function __construct()
    {
        $this->middleware(['auth', 'verified', 'profiled', 'not_blocked']);
        $this->authorizeResource(ChecklistBalk::class, 'checklists_balk');
    }

    /**
     * Display a listing of the resource.
     */
    public function index(ChecklistBalkFilterRequest $request): Response
    {

        $data = $request->validated();

        $filter = app()->make(ChecklistBalkFilter::class, ['queryParams' => array_filter($data)]);

        $checklists = ChecklistBalk::filter($filter)
            ->orderBy('created_at', 'desc')
            ->paginate(config('backend.checklists_balk.pagination'));

        return \response()->view('backend.checklists.balk.index', [
    ...
    public function create(): Response
    {
        ...
        $equipment_name_ids = TrkEquipment::where('system_id', $condition_system->id)
            ->whereIn('trk_room_id', $trk_room_ids)->pluck('equipment_name_id')->toArray();
        ...
        return \response()->view('backend.checklists.balk.create', [
     ...
     */
    public function store(StoreChecklistBalkFormRequest $request): RedirectResponse
    {
        Log::info('user try to store new checklist Balk', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
        ]);
        ...
        if ($request->isMethod('post')) {
            try {
                ...
                if (empty($trk_equipment)) {
                    return redirect()->back()->with('error', 'Нет такой балки на выбранном ТРК и в этом помещении')->withInput();
                }

                DB::beginTransaction();

                $checklist = ChecklistBalk::create([
                    'trk_room_id' => $trk_room->id,
                    'trk_equipment_id' => $trk_equipment->id,
                    'balk_number' => 1,
                    'balk_size_type' => $data['balk_size_type'],
                    ...
                    'air_flap' => $data['air_flap'],
                    'cold_water_inlet_temperature' => $data['cold_water_inlet_temperature'],
                    'cold_water_outlet_temperature' => $data['cold_water_outlet_temperature'],
                    'cold_water_pressure_drop' => $data['cold_water_pressure_drop'],
                    'cold_water_valve' => $data['cold_water_valve'],
                    'cold_water_rate' => $data['cold_water_rate'],
                ...
                return redirect()->route('checklists_balk.index')->with('success', 'Данные сохранены.');
    ...
    {
        $this->authorize('create', ChecklistBalk::class);

        $work_types = WorkName::whereIn('name', WorkName::CHECKLIST_WORK_TYPES)->get();

        return \response()->view('backend.checklists.balk.create_from_trk_equipment', [
            'executors' => User::orderBy('name')->get(),
            'work_types' => $work_types,
     ...
     * Store a newly created resource in storage.
     */
    public function store_from_trk_equipment(StoreChecklistBalkFromTrkEquipmentFormRequest $request, TrkEquipment $trk_equipment): RedirectResponse
    {
        $this->authorize('create', ChecklistBalk::class);

        Log::info('user try to store new checklist Balk from trk equipment', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
            'trk_equipment' => $trk_equipment,
        ]);
    ...
    public function create_from_trk_room(TrkRoom $trk_room): Response
    {
        ...
        return \response()->view('backend.checklists.balk.create_from_trk_room', [
            'executors' => User::orderBy('name')->get(),
            'work_types' => $work_types,
     ...
     * Store a newly created resource in storage.
     */
    public function store_from_trk_room(StoreChecklistBalkFromTrkRoomFormRequest $request, TrkRoom $trk_room): RedirectResponse
    {
        $this->authorize('create', ChecklistBalk::class);

        Log::info('user try to store new checklist Balk from trk room', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
            'trk_room' => $trk_room,
        ]);
        ...
        if ($request->isMethod('post')) {
            try {
                ...
                $system_balk = System::where('name', System::AIR_CONDITION)->first();
                ...
                foreach ($data['equipments'] as $equipment) {
                    ...
                    $checklist = ChecklistBalk::create([
                        ...
                        'trk_equipment_id' => $trk_equipment->id,
                        'balk_number' => 1,
                        'balk_size_type' => $equipment['balk_size_type'],
                        ...
                        'air_flap' => $equipment['air_flap'],
                        'cold_water_inlet_temperature' => $equipment['cold_water_inlet_temperature'],
                        'cold_water_outlet_temperature' => $equipment['cold_water_outlet_temperature'],
                        'cold_water_pressure_drop' => $equipment['cold_water_pressure_drop'],
                        'cold_water_valve' => $equipment['cold_water_valve'],
                        'cold_water_rate' => $equipment['cold_water_rate'],
     ...
     */
    public function show(ChecklistBalk $checklists_balk): Response
    {
        return \response()->view('backend.checklists.balk.show', [
            'checklist' => $checklists_balk,
        ]);
    }
     ...
     */
    public function edit(ChecklistBalk $checklists_balk): Response
    {

        $work_types = WorkName::whereIn('name', ['ТО 4', 'ТО 5', 'ТО 6'])->get();
        $system_balk = System::where('name', System::AIR_CONDITION)->first();

        $trk_building_ids = TrkRoom::where('trk_id', $checklists_balk->trk_room->trk->id)
            ->groupBy('building_id')
            ->pluck('building_id')
            ->toArray();
        ...
        $trk_room_ids = TrkRoom::where('trk_id', $checklists_balk->trk_room->trk->id)
            ->pluck('room_id')
            ->toArray();

        $rooms = Room::whereIn('id', $trk_room_ids)->orderBy('name')->get();

        $trk_floor_ids = TrkRoom::where('trk_id', $checklists_balk->trk_room->trk->id)
            ->groupBy('floor_id')
            ->pluck('floor_id')
            ->toArray();
        ...
        return \response()->view('backend.checklists.balk.edit', [
            'checklist' => $checklists_balk,
            'trks' => Trk::orderBy('sort_order')->get(),
     ...
     */
    public function update(UpdateChecklistBalkFormRequest $request, ChecklistBalk $checklists_balk): RedirectResponse
    {

        Log::info('user try to update checklist Balk', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
            'checklist Balk' => $checklists_balk
        ]);
        ...
        if ($request->isMethod('patch')) {
            try {
                ...
                $trk_room = TrkRoom::where('trk_id', $checklists_balk->trk_room->trk->id)
                    ->where('building_id', $data['building_id'])
                    ->where('floor_id', $data['floor_id'])
                    ->where('room_id', $data['room_id'])
                    ->first();
                ...
                if (empty($trk_room->id)) {
                    ...
                    $trk = Trk::find($checklists_balk->trk_room->trk->id);
                ...
                if (empty($equipment_name->id)) {
                    ...
                    return redirect()->back()->with('error', 'Нет балки с таким названием: ' . $data['equipment_id'])->withInput();
                }
                ...
                if (empty($trk_equipment->id)) {
                    ...
                    return redirect()->back()->with('error', 'Нет такой балки: ' . $trk_room->trk->name . ', ' . $trk_room->building->name . ', ' . $trk_room->floor->name . ' - ' . $equipment_name->name)->withInput();
                }

                DB::beginTransaction();

                $avr = $checklists_balk->avr->first();

                foreach($avr->avr_equipments as $avr_equipment)
                {
                    if(
                        $avr_equipment->trk_equipment_id == $checklists_balk->trk_equipment_id
                ...
                AvrWork::where('avr_id', $avr->id)
                    ->where('trk_equipment_id', $checklists_balk->trk_equipment_id)
                    ->forceDelete();
     ...
     */
    public function destroy(ChecklistBalk $checklists_balk): RedirectResponse
    {
        Log::info('user try to delete ChecklistBalk', [
            'user' => Auth::user()->name,
        ...
        try {
            ...
            $avr_id = $checklists_balk->avr->first()->id;

            DocCommunication::where('from_id', $checklists_balk->id)
                ->where('from_type', ChecklistBalk::class)
                ->where('to_id', $avr_id)
            ...
            $checklists_balk->delete();

            DB::commit();

            return redirect()->route('checklists_balk.index')->with('success', 'Данные удалены.');
