...
use App\Http\Filters\Checklists\AirDuct\ChecklistAirDuctFilter;
use App\Http\Requests\Checklists\AirDuct\ChecklistAirDuctFilterRequest;
use App\Http\Requests\Checklists\AirDuct\StoreChecklistAirDuctFormRequest;
use App\Http\Requests\Checklists\AirDuct\StoreChecklistAirDuctFromTrkEquipmentFormRequest;
use App\Http\Requests\Checklists\AirDuct\StoreChecklistAirDuctFromTrkRoomFormRequest;
use App\Http\Requests\Checklists\AirDuct\UpdateChecklistAirDuctFormRequest;
...
class ChecklistAirDuctController extends Controller
{
    public function __construct()
    {
        $this->middleware(['auth', 'verified', 'profiled', 'not_blocked']);
        $this->authorizeResource(ChecklistAirDuct::class, 'checklists_air_duct');
    }

    /**
     * Display a listing of the resource.
     */
    public function index(ChecklistAirDuctFilterRequest $request): Response
    {

        $data = $request->validated();

        $filter = app()->make(ChecklistAirDuctFilter::class, ['queryParams' => array_filter($data)]);

        $checklists = ChecklistAirDuct::filter($filter)
            ->orderBy('created_at', 'desc')
            ->paginate(config('backend.checklists_air_duct.pagination'));

        return \response()->view('backend.checklists.air_duct.index', [
    ...
    public function create(): Response
    {
        ...
        return \response()->view('backend.checklists.air_duct.create', [
     ...
     */
    public function store(StoreChecklistAirDuctFormRequest $request): RedirectResponse
    {
        Log::info('user try to store new checklist air duct', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
        ]);
        ...
        if ($request->isMethod('post')) {
            try {
                ...
                foreach ($data['equipments'] as $equipment) {
                    ...
                    $checklist = ChecklistAirDuct::create([
                        'trk_room_id' => $trk_room->id,
                        ...
                        'duct_cross_sectional_area' => $equipment['duct_cross_sectional_area'],
                ...
                return redirect()->route('checklists_air_duct.index')->with('success', 'Данные сохранены.');
    ...
    {
        $this->authorize('create', ChecklistAirDuct::class);
        ...
        return \response()->view('backend.checklists.air_duct.create_from_trk_equipment', [
            'executors' => User::orderBy('name')->get(),
     ...
     * Store a newly created resource in storage.
     */
    public function store_from_trk_equipment(StoreChecklistAirDuctFromTrkEquipmentFormRequest $request, TrkEquipment $trk_equipment): RedirectResponse
    {
        $this->authorize('create', ChecklistAirDuct::class);

        Log::info('user try to store new checklist air duct from trk equipment', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
            'trk_equipment' => $trk_equipment,
        ]);
    ...
    public function create_from_trk_room(TrkRoom $trk_room): Response
    {
        ...
        return \response()->view('backend.checklists.air_duct.create_from_trk_room', [
            'executors' => User::orderBy('name')->get(),
            'work_types' => $work_types,
     ...
     * Store a newly created resource in storage.
     */
    public function store_from_trk_room(StoreChecklistAirDuctFromTrkRoomFormRequest $request, TrkRoom $trk_room): RedirectResponse
    {
        $this->authorize('create', ChecklistAirDuct::class);

        Log::info('user try to store new checklist air duct from trk room', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
            'trk_room' => $trk_room,
        ]);
     ...
     */
    public function show(ChecklistAirDuct $checklists_air_duct): Response
    {
        return \response()->view('backend.checklists.air_duct.show', [
            'checklist' => $checklists_air_duct,
        ]);
    }
     ...
     */
    public function edit(ChecklistAirDuct $checklists_air_duct): Response
    {
        $work_types = WorkName::whereIn('name', ['ТО 4', 'ТО 5', 'ТО 6'])->get();
        $system = System::where('name', System::AIR_RECYCLE)->first();

        $equipments = TrkEquipment::where('id', $checklists_air_duct->trk_equipment->id)
            ->where('system_id', $system->id)
            ->get();

        return \response()->view('backend.checklists.air_duct.edit', [
            'checklist' => $checklists_air_duct,
            'trks' => Trk::orderBy('sort_order')->get(),
     ...
     */
    public function update(UpdateChecklistAirDuctFormRequest $request, ChecklistAirDuct $checklists_air_duct): RedirectResponse
    {

        Log::info('user try to update checklist air duct', [
            'user' => Auth::user()->name,
            'request' => $request->all(),
            'checklist air duct' => $checklists_air_duct
        ]);
        ...
        if ($request->isMethod('patch')) {
            try {
                ...
                $avr = $checklists_air_duct->avr->first();

                foreach($avr->avr_equipments as $avr_equipment)
                {
                    if(
                        $avr_equipment->trk_equipment_id == $checklists_air_duct->trk_equipment_id
                ...
                AvrWork::where('avr_id', $avr->id)
                    ->where('trk_equipment_id', $checklists_air_duct->trk_equipment_id)
                    ->forceDelete();
                ...
                $checklists_air_duct->update([
                    ...
                    'duct_cross_sectional_area' => $data['duct_cross_sectional_area'],
     ...
     */
    public function destroy(ChecklistAirDuct $checklists_air_duct): RedirectResponse
    {
        Log::info('user try to delete ChecklistAirDuct', [
            'user' => Auth::user()->name,
        ...
        try {
            ...
            $avr_id = $checklists_air_duct->avr->first()->id;

            DocCommunication::where('from_id', $checklists_air_duct->id)
                ->where('from_type', ChecklistAirDuct::class)
                ->where('to_id', $avr_id)
            ...
            $checklists_air_duct->delete();

            DB::commit();

            return redirect()->route('checklists_air_duct.index')->with('success', 'Данные удалены.');
