...
@section('title', 'Просмотр | Статус заявки в администрацию')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">Статус заявки в администрацию</h4>
                    </div>
                </div>
            </div>
                        ...
                        <div class="col">
                            <div class="card shadow p-3">
                                <ul class="list-group mb-3">
                                    <li class="list-group-item"><b>Название: </b>{{$admin_app_status->name}}</li>
                                </ul>
                                <div class="btn-group btn-group-sm col-12 col-md-6" role="group"
                                    ...
                                    <a href="{{route('admin_app_statuses.index')}}"
                                       class="btn btn-outline-success col-4 col-md-2 rounded me-1"><img
                                            ...
                                            title="Назад"></a>
                                    @if(auth()->user()->can('edit') || Auth::id() === $admin_app_status->author->id)
                                        <a href="{{route('admin_app_statuses.edit', $admin_app_status)}}"
                                           class="btn btn-outline-warning col-4 col-md-2 rounded me-1"><img
                                                src="{{asset('assets/images/backend/svg/pencil.svg')}}" alt="edit"
                                                title="Редактировать"></a>
                                    @endif
                                    @if(auth()->user()->can('delete') || Auth::id() === $admin_app_status->author->id)
                                        <form action="{{route('admin_app_statuses.destroy', $admin_app_status)}}" method="post">
                                            @csrf
                                            @method('delete')
