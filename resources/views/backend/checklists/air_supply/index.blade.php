...
@section('title', 'Главная | Чеклист приточных установок')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        <h4 class="d-inline-block me-3">Чеклист приточных установок</h4>
                        @if(auth()->user()->can('checklist_air_supply create'))
                            <a href="{{route('checklists_air_supply.create')}}"><img
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="card shadow">
                                <div class="card-body">
                                    <a href="{{route('checklists_air_supply.export_blank')}}" class="btn btn-sm btn-outline-success mb-3">Бланк чеклиста</a>
                                    ...
                                    <table class="table table-striped table-hover shadow table-bordered">
                                        ...
                                        <form action="{{route('checklists_air_supply.index')}}" method="get">
                                            @csrf
                                            <tr>
                                                <td>
                                        ...
                                        @forelse($checklists as $checklists_air_supply)
                                            @if(auth()->user()->can('read'))
                                                <tr onclick="window.location='{{ route('checklists_air_supply.show', $checklists_air_supply) }}';">
                                            @else
                                                <tr>
                                                    @endif
                                                    <td class="text-nowrap">{{$checklists_air_supply->created_at}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_supply->trk_equipment->trk_room->trk->name}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_supply->trk_equipment->trk_room->room->name}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_supply->trk_equipment->equipment_name->name}}</td>
                                                </tr>
                                                @empty
                                                    <tr>
