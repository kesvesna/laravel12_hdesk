...
@section('title', 'Главная | Чеклист вытяжных установок')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        <h4 class="d-inline-block me-3">Чеклист вытяжных установок</h4>
                        @if(auth()->user()->can('checklist_air_extract create'))
                            <a href="{{route('checklists_air_extract.create')}}"><img
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="card shadow">
                                <div class="card-body">
                                    <a href="{{route('checklists_air_extract.export_blank')}}" class="btn btn-sm btn-outline-success mb-3">Бланк чеклиста</a>
                                    ...
                                    <table class="table table-striped table-hover shadow table-bordered">
                                        ...
                                        <form action="{{route('checklists_air_extract.index')}}" method="get">
                                            @csrf
                                            <tr>
                                                <td>
                                        ...
                                        @forelse($checklists as $checklists_air_extract)
                                            @if(auth()->user()->can('read'))
                                                <tr onclick="window.location='{{ route('checklists_air_extract.show', $checklists_air_extract) }}';">
                                            @else
                                                <tr>
                                                    @endif
                                                    <td class="text-nowrap">{{$checklists_air_extract->created_at}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_extract->trk_equipment->trk_room->trk->name}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_extract->trk_equipment->trk_room->room->name}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_extract->trk_equipment->equipment_name->name}}</td>
                                                </tr>
                                                @empty
                                                    <tr>
