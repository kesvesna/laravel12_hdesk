            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        <h4 class="d-inline-block me-3">Чеклист кондиционера</h4>
                        @if(auth()->user()->can('checklist_conditioner create'))
                            <a href="{{route('checklists_conditioner.create')}}"><img
                            ...
                            <div class="card shadow">
                                <div class="card-body">
                                    <table class="table table-striped table-hover shadow table-bordered">
                                        ...
                                        <form action="{{route('checklists_conditioner.index')}}" method="get">
                                            @csrf
                                            <tr>
                                        ...
                                        @forelse($checklists as $checklists_conditioner)
                                            @if(auth()->user()->can('read'))
                                                <tr onclick="window.location='{{ route('checklists_conditioner.show', $checklists_conditioner) }}';">
                                            @else
                                                <tr>
                                                    @endif
                                                    <td class="d-none d-md-table-cell">{{$checklists_conditioner->created_at}}</td>
                                                    <td class="d-none d-md-table-cell">{{$checklists_conditioner->trk_room->trk->name}}</td>
                                                    <td>{{$checklists_conditioner->trk_room->room->name}}</td>
                                                    <td>{{$checklists_conditioner->trk_equipment->equipment_name->name}}</td>
                                                </tr>
                                                @empty
                                                    <tr>
