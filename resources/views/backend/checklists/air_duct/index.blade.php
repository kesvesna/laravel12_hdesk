...
@section('title', 'Главная | Чеклист воздуховода')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        <h4 class="d-inline-block me-3">Чеклисты воздуховодов</h4>
                        @if(auth()->user()->can('checklist_air_duct create'))
                            <a href="{{route('checklists_air_duct.create')}}"><img
                            ...
                            <div class="card shadow">
                                <div class="card-body">
                                    ...
                                    <table class="table table-striped table-hover shadow table-bordered">
                                        ...
                                        <form action="{{route('checklists_air_duct.index')}}" method="get">
                                            @csrf
                                            <tr>
                                                <td>
                                        ...
                                        @forelse($checklists as $checklists_air_duct)
                                            @if(auth()->user()->can('read'))
                                                <tr onclick="window.location='{{ route('checklists_air_duct.show', $checklists_air_duct) }}';">
                                            @else
                                                <tr>
                                                    @endif
                                                    <td class="text-nowrap">{{$checklists_air_duct->created_at}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_duct->trk_room->trk->name}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_duct->trk_room->room->name}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_duct->trk_equipment->equipment_name->name}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_duct->air_direction_type == 1 ? 'вытяжка' : 'приток'}}</td>
                                                    <td class="text-nowrap">{{$checklists_air_duct->measuring_point_number}}</td>
                                                </tr>
                                                @empty
                                                    <tr>
