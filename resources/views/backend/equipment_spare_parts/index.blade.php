...
@section('title', 'Главная | Запчасти оборудования')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        <h4 class="d-inline-block me-3">Запчасти оборудования</h4>
                        @if(auth()->user()->can('create'))
                            <a href="{{route('equipment_spare_parts.create')}}"><img
                                ...
                                <div class="card-body">
                                    <table class="table table-striped table-hover shadow">
                                        ...
                                        @if(auth()->user()->can('read'))
                                            <form action="{{route('equipment_spare_parts.index')}}" method="get">
                                                <tr>
                                                    <td>
                                                        @csrf
                                            ...
                                            @forelse($equipment_spare_parts as $equipment_spare_part)
                                                <tr onclick="window.location='{{ route('equipment_spare_parts.show', $equipment_spare_part->id) }}';">
                                                    <td>{{$equipment_spare_part->trk->name}}</td>
                                                    <td>{{$equipment_spare_part->store_house_name->name}}</td>
                                                    <td>{{$equipment_spare_part->spare_part_name->name}}</td>
                                                    <td>{{$equipment_spare_part->spare_part_model}}</td>
                                    ...
                                    {{$equipment_spare_parts->withQueryString()->links()}}
                                </div>
                            </div>
                        </div>
                    </div>
