                        ...
                        <div class="col">
                            <div class="card shadow p-3">
                                <ul class="list-group mb-3">
                                    <li class="list-group-item"><b>Название: </b>{{$parameter_name->name}}</li>
                                </ul>
                                <div class="btn-group btn-group-sm col-12 col-md-6" role="group"
                                    ...
                                    <a href="{{route('parameter_names.index')}}"
                                       class="btn btn-outline-success col-4 col-md-2 rounded me-1"><img
                                            ...
                                            title="Назад"></a>
                                    @if(auth()->user()->can('edit') || Auth::id() == $parameter_name->author->id)
                                        <a href="{{route('parameter_names.edit', $parameter_name)}}"
                                           class="btn btn-outline-warning col-4 col-md-2 rounded me-1"><img
                                                src="{{asset('assets/images/backend/svg/pencil.svg')}}" alt="edit"
                                                title="Редактировать"></a>
                                    @endif
                                    @if(auth()->user()->can('delete') || Auth::id() === $parameter_name->author->id)
                                        <form action="{{route('parameter_names.destroy', $parameter_name)}}"
                                              method="post">
                                            @csrf
                                            @method('delete')
