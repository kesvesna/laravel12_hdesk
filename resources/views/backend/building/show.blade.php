...
@section('title', 'Просмотр | Блок/Зона')
                        ...
                        <div class="col">
                            <div class="card shadow p-3">
                                <ul class="list-group mb-3">
                                    <li class="list-group-item"><b>Название: </b>{{$building->name}}</li>
                                    ...
                                    @if(count($trk_rooms) > 0)
                                        ...
                                        @endforelse
                                        @role('sadmin')
                                        <form action="{{route('trk_room.change_building_in_rooms', $building)}}" method="post">
                                            @csrf
                                            @method('patch')
                                                ...
                                                <div class="work-add-div mb-1 mb-md-0">
                                                    <div class="row row-cols-1">
                                                        <div class="col-12 col-md-5">
                                                           <select required name="building_id" class="form-select form-select-sm work-type-select">
                                                                    <option value="">не заменять</option>
                                                                    @forelse($buildings as $building_2)
                                                                        <option value="{{$building_2->id}}">{{$building_2->name}}</option>
                                                                    @empty
