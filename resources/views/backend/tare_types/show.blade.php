...
@section('title', 'Просмотр | Тип тары')
                        ...
                        <div class="col">
                            <div class="card shadow p-3">
                                <ul class="list-group mb-3">
                                    <li class="list-group-item"><b>Название: </b>{{$tare_type->name}}</li>
                                </ul>
                                <div class="btn-group btn-group-sm col-12 col-md-6" role="group"
                                    ...
                                    <a href="{{route('tare_types.index')}}"
                                       class="btn btn-outline-success col-4 col-md-2 rounded me-1"><img
                                            ...
                                            title="Назад"></a>
                                    @if(auth()->user()->can('edit') || Auth::id() === $tare_type->author->id)
                                        <a href="{{route('tare_types.edit', $tare_type)}}"
                                           class="btn btn-outline-warning col-4 col-md-2 rounded me-1"><img
                                                src="{{asset('assets/images/backend/svg/pencil.svg')}}" alt="edit"
                                                title="Редактировать"></a>
                                    @endif
                                    @if(auth()->user()->can('delete') || Auth::id() === $tare_type->author->id)
                                        <form action="{{route('tare_types.destroy', $tare_type)}}" method="post">
                                            @csrf
                                            @method('delete')
