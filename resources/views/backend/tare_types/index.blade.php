...
@section('title', 'Главная | Типы тары')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        <h4 class="d-inline-block me-3">Типы тары</h4>
                        @if(auth()->user()->can('create'))
                            <a href="{{route('tare_types.create')}}"><img
                            ...
                            <div class="card shadow">
                                <div class="card-body">
                                    <table class="table table-striped table-hover shadow">
                                        ...
                                        <tr>
                                            <td>
                                                <form action="{{route('tare_types.index')}}" method="get">
                                                    @csrf
                                                    ...
                                                    <datalist id="datalistOptions">
                                                        @forelse($all_types as $tare_type)
                                                            <option value="{{$tare_type->name}}">
                                                        @empty
                                        ...
                                        @forelse($types as $type)
                                            @if(auth()->user()->can('read'))
                                                <tr onclick="window.location='{{ route('tare_types.show', $type->id) }}';">
                                            @else
                                                <tr>
                                                    @endif
                                                    <td>{{$type->name}}</td>
