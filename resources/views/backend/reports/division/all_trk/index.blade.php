...
@section('title', 'Главная | Отчет по подразделению')
                        ...
                        <div class="col">
                            <div class="card shadow">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-12">
                                            <form action="{{route('division_reports.all_trk.report')}}" method="post">
                                                @csrf
                                                @method('post')
                                                        ...
                                                        <td>
                                                            ...
                                                            <select required name="user_division_id" class="form-select form-select-sm" id="user_division_id">
                                                                @forelse($divisions as $division)
                                                                    <option value="{{$division->id}}" {{old('user_division_id') == $division->id ? 'selected' : null}}>{{$division->name}}</option>
                                                                @empty
