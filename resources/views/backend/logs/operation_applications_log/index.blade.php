...
@section('title', 'Главная | Логи заявок в эксплуатацию')
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        <h4 class="d-inline-block me-3">Логи заявок в эксплуатацию</h4>
                        @if(auth()->user()->can('operation_application create'))
                            <a href="{{route('operation_applications_log.create')}}"><img
...
{{--                                                    data-bs-toggle="modal" data-bs-target="#operation_applications_log">Выгрузка заявок--}}
{{--                                            </button>--}}
{{--                                        </div>--}}
{{--                                    </div>--}}
{{--                                </div>--}}
                                <div class="card-body">
                                    ...
                                    <table class="table table-striped table-hover shadow">
                                        ...
                                        <tr>
                                            <th>ID</th>
                                            <th class="text-nowrap">Создана</th>
                                            <th class="text-nowrap">Редакция</th>
                                            <th class="text-nowrap">ТРК</th>
                                            <th class="text-nowrap">Подразделение</th>
                                            <th>Проблема</th>
                                            <th>Выполнено, %</th>
                                            <th class="text-nowrap">Done_author</th>
                                        </tr>
                                        </thead>
                                        <tbody style="cursor: pointer;">
                                        <form action="{{route('operation_applications_log.index')}}" method="get">
                                            @csrf
                                            <tr>
                                                <td>
                                                    <input class="form-control form-control-sm" type="search"
                                                           id="operation_application_id" placeholder="Поиск ..."
                                                           onchange="this.form.submit();" name="operation_application_id"
                                                           value="{{$old_filters['operation_application_id'] ?? null}}">
                                                </td>
                                                <td>
                                        ...
                                        @forelse($operation_applications_log as $operation_application)
                                            @if(auth()->user()->can('read'))
                                                <tr onclick="window.location='{{ route('operation_applications_log.show', $operation_application->id) }}';">
                                            @else
                                                <tr>
                                                    @endif
                                                    <td>{{$operation_application->operation_application_id}}</td>
                                                    <td class="text-nowrap">{{date('d-m-Y H:i', strtotime($operation_application->created_at))}}</td>
                                                    <td class="text-nowrap">{{date('d-m-Y H:i', strtotime($operation_application->updated_at))}}</td>
                                                    <td class="text-nowrap">{{$operation_application->trk->name}}</td>
                                                    <td class="text-nowrap">{{$operation_application->division->name}}</td>
                                                    ...
                                                    <td class="text-nowrap">{{$operation_application->done_author->name ?? 'null'}}</td>
                                                </tr>
                                                @empty
                                                    <tr>
                                    ...
                                    {{$operation_applications_log->withQueryString()->links()}}
                                </div>
                            </div>
                        </div>
                    </div>
...
{{--        <div class="modal fade" id="operation_applications_log" tabindex="-1"--}}
{{--             aria-labelledby="operation_applications_log" aria-hidden="true">--}}
...
{{--                        <form action="{{route('operation_applications_log.export')}}" method="post">--}}
{{--                            @csrf--}}
{{--                            @method('post')--}}
