...
<head>
    ...
    <meta charset="utf-8"/>
    <title>Подтверждение почты | FG</title>
                ...
                <div class="col-lg-12">
                    <div class="card overflow-hidden">
                        <div class="row g-0">
                            ...
                            <div class="col-lg-6">
                                <div class="p-lg-5 my-3">
                                    @if (session('status') == 'verification-link-sent')
                                        <div class="alert alert-success text-center m-4" role="alert">
                                            {{'Ссылка для подтверждения почты отправлена Вам!'}}
                                        </div>
                                    @else
                                        <div class="alert alert-warning text-center m-4" role="alert">
                                            {{'В почте нет ссылки для подтверждения?'}}
                                        </div>
                                    @endif
                                    <form method="POST" action="{{ route('verification.send') }}"
                                          class="text-center my-5">
                                        @csrf
                                        <button type="submit" class="btn btn-primary">Отправить ссылку еще раз</button>
                                    </form>
                                    @if (session('status') == 'verification-link-sent')
                                        {{Session::forget('status')}}
                                        <p class="mx-4">Ссылка в почте есть?</p>
                                        <p class="mx-4">Можно закрыть это окно.</p>
                                    @endif
                                </div>
                            </div>
                            <!-- end col -->
